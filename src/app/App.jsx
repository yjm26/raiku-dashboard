...
function LoadingState() { return <main className="mx-auto grid min-h-screen max-w-xl place-items-center px-4" aria-busy="true"><div className="w-full border border-rule bg-surface p-6" role="status"><p className="m-0 font-mono text-xs uppercase text-muted">Raiku / loading</p><h1 className="mt-3 text-xl font-bold">Loading dashboard snapshot</h1><p className="text-sm text-muted">Fetching the latest rkuSOL holder data.</p></div></main>; }
function ErrorState({ error, onRetry }) { return <main className="mx-auto grid min-h-screen max-w-xl place-items-center px-4" aria-busy="false"><div className="w-full border-2 border-rule bg-surface p-6" role="alert"><p className="m-0 font-mono text-xs uppercase text-muted">Snapshot unavailable</p><h1 className="mt-3 text-xl font-bold">Couldn&apos;t load the dashboard snapshot</h1><p className="text-sm text-muted">{error.message || 'The snapshot could not be loaded right now.'}</p><button className="mt-4 border border-rule bg-ink px-4 py-2 text-sm font-semibold text-white" type="button" onClick={onRetry}>Try again</button></div></main>; }
...
export default function App() {
  ...
  return <AppShell><TopBar snapshot={snapshot} /><div className="mt-4 grid gap-4 lg:grid-cols-[minmax(280px,0.5fr)_minmax(0,1.5fr)]"><ProtocolInfo snapshot={snapshot} /><main className="app-main min-w-0" aria-busy="false"><DashboardHeader snapshot={snapshot} /><MetricGroup primary={primary} secondary={secondary} /><InsightGrid snapshot={snapshot} /><WalletSearch rows={snapshot.realRows} /><DataSection rows={snapshot.realRows} /><MethodologyNote coverage={snapshot.coverage} /></main></div></AppShell>;
}
