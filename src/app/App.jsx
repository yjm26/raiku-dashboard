...
export default function App() {
  ...
  const tvlUsdValue = Number(stats.tvlUsd); const tvlUsdLabel = Number.isFinite(tvlUsdValue) && tvlUsdValue > 0 ? `$${formatNumber(tvlUsdValue, { maximumFractionDigits: 0 })}` : null;
  ...
  const secondary = [{ label: 'APY', value: apyLabel, detail: 'latest reported rate' }, { label: 'TVL', value: tvlSolLabel, detail: tvlUsdLabel ?? 'total value locked', tone: 'green' }, { label: 'rkuSOL rate', value: rateLabel, detail: 'SOL per rkuSOL', tone: 'blue' }, { label: 'Daily points', value: formatNumber(stats.dailyPoints, { maximumFractionDigits: 0 }), detail: 'estimated daily accrual', tone: 'green' }, { label: 'Official holders', value: formatNumber(stats.officialHolders, { maximumFractionDigits: 0 }), detail: 'Raiku API reference' }, { label: 'Top-10 concentration', value: stats.top10Share == null ? '—' : `${formatNumber(stats.top10Share, { maximumFractionDigits: 1 })}%`, detail: 'share of total supply', tone: 'amber' }];
